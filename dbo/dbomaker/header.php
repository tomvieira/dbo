...
$(document).ready(function(){
	...
	$(document).on('click', '#wrapper-modules > .anchor a', function(e){
		...
		if($(this).hasClass('locked')) {
			...
			ajaxLoad('#wrapper-fields', 'actions.php?showModule='+$(this).attr('href'), init);
		}
	})
	...
	$(document).on('click', '.wrapper-module .fieldset h2', function(e){
		...
		else
		{
			...
			$(this).addClass('active');
			$(this).closest('.fieldset').find('.anchor').fadeIn('fast');
		}
	})
	...
	//making TAB tabulate
	$(document).on('keydown', 'textarea.code', function(e){
		if(e.keyCode == 9)
		{
			e.preventDefault();
			insertAtCaret(this, '\t');
	...
	$(document).on('click', '.new-field', function(){
		...
		ajaxLoad('#wrapper-details', 'actions.php?getNewFieldForm='+$(this).attr('rel'));
	})

	//clicks nos tipos de campo
	$(document).on('click', '.field-types li', function(){
		$('.field-types li').removeClass('active');
		$(this).find('input').attr('checked', 'checked');
		$(this).addClass('active');
	})

	//fazendo os botões de salvar aparecerem na hora certa...
	$(document).on('change', '#form-field input, #form-field textarea, #form-field select', function(){
		if($('#wrapper-details .button-next').is(':hidden'))
		{
			$('#wrapper-details .button-next').fadeIn('slow');
		}
	})
	$(document).on('change', '#form-module input, #form-module textarea, #form-module select', function(){
		if($('#wrapper-fields .button-next').is(':hidden'))
		{
			$('#wrapper-fields .button-next').fadeIn('slow');
		}
	})
				...
				function(data){
					...
					if(partes[0] == 'RUN_UPDATE_FIELD_OK')
					{
						ajaxLoad('#wrapper-fields', 'actions.php?showModule='+partes[1], function(){
							$('#wrapper-fields .field-'+partes[2]).fadeTo('slow', 0, function(){
								$('#wrapper-fields .field-'+partes[2]).addClass('active');
								$('#wrapper-fields .field-'+partes[2]).fadeTo('slow', 1);
								ajaxLoad('#wrapper-details', 'actions.php?showField='+partes[1]+'||'+partes[2], init);
							});
						});
					//updated a field successfully
					}else if(partes[0] == 'RUN_UPDATE_MODULE_OK') {
						ajaxLoad('#wrapper-modules > .anchor', 'actions.php?showModules=1', function(){
							$('#wrapper-modules .module-'+partes[1]).fadeTo('slow', 0, function(){
								$('#wrapper-modules .module-'+partes[1]).addClass('active');
								$('#wrapper-modules .module-'+partes[1]).fadeTo('slow', 1);
								ajaxLoad('#wrapper-fields', 'actions.php?showModule='+partes[1], init);
							});
						});
	...
	$(document).on('click', '.new-module', function(e){
		...
		ajaxLoad('#wrapper-fields', 'actions.php?runNewModule=1', function(){
			...
			$('#anchor-module-basic-info').fadeIn('fast', function(){
