...
function showModules ()
{
	...
	?>
		<a title='Order: <?= $modulo->order_by ?>' href='<?= $modulo->modulo ?>' class='sortable draggable <?= $class ?> module-<?= $modulo->modulo ?> module' module='<?= $modulo->modulo ?>' id='module-<?= encNameAjax($modulo->modulo) ?>'><?= $modulo->titulo ?></a>
	<?
	}
	?><div class='new-module button-new'>Novo Módulo</div><?
}
	...
	?>
		<div class='wrapper-module module-<?= $mod ?>'>
			...
			<div id='menu-fields'>
				...
				<div class='new-field button-new' rel='<?= $mod ?>'>Novo Campo</div>
			</div>

			<div class='button-next hidden' rel='#form-module' sending='Enviando...' original_value='Salvar'><span>Salvar</span></div>
	...
	?>
		<form id='form-module' action='actions.php'>
			...
			<div id='module-main' class='standard fieldset'>
				<h2 id='h2-module-basic-info'>Informações Básicas</h2>
				...
				<div class='anchor' id='anchor-module-basic-info'>
					...
					<div class='row'>
						<div class='item'>
							<label>Gênero</label>
							<div class='input'>
								<input type='radio' name='genero' value='o' <?= ($module->genero == 'o')?('CHECKED'):('') ?>> Masc. &nbsp;&nbsp;&nbsp;
								<input type='radio' name='genero' value='a' <?= ($module->genero == 'a')?('CHECKED'):('') ?>> Fem.
							</div>
						</div>
					</div><!-- row -->
...
function getFieldForm ($mod,$field)
{
	...
	<form id='form-field' action='actions.php'>
		...
		<div id='field-main' class='standard fieldset'>
			<h2 class='<?= ($_SESSION['dbomaker_controls']['show_field_basic'])?('active'):('') ?> toggle-control' rel='show_field_basic'>Informações Básicas</h2>
		...
		<div id='field-functions' class='standard fieldset'>
			<h2 class='<?= ($_SESSION['dbomaker_controls']['show_field_advanced'])?('active'):('') ?> toggle-control' rel='show_field_advanced'>Informações Avançadas</h2>
		...
		<div id='field-type' class='standard fieldset'>
			<h2 class='<?= ($_SESSION['dbomaker_controls']['show_field_type'])?('active'):('') ?> toggle-control' rel='show_field_type'>Tipo do Campo</h2>
		...
		<div class='button-next hidden' rel='#form-field' sending='Enviando...' original_value='Salvar'><span>Salvar</span></div>
									...
									<div class='item'>
										...
										<?
											...
											foreach($params as $chave => $valor)
											{
												...
												echo $partes[0].":<span class='param-desc'>".$partes[1]."</span><br/>";
											}
										?>
										</label>
										<div class='input'>
	...
	<div class='wrapper-new-field-type'>
		<form method='POST' action='actions.php' id='form-new-field'>
			<ul class='field-types field-types-basic'>
				...
				<li><input type='radio' name='tipo' value='custom'/><span>Personalizado</span></li>
			</ul>

			<ul class='field-types field-types-basic'>
				<li class='title'>Automáticos</li>
				<li><input type='radio' name='tipo' value='created_by'/><span>Criado Por (created_by)</span></li>
			...
			<div class='button-next' rel='#form-new-field' sending='Enviando...' original_value='Prosseguir &raquo;'><span>Prosseguir &raquo;</span></div>
...
function syncAll()
{
	...
	if(is_array($_SESSION['dbomaker_modulos']))
	{
		...
		{
			if($key != 'temporary_module_key_5658')
			{
...
function writeModuleFile($mod)
{
	...
	if($fh = fopen('../'.$file_prefix.$mod.$file_sufix, 'w'))
	{
		...
		fwrite($fh, "\$module->order_by = '".$module->order_by."';\n");
